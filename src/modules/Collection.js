...
export class Collection {
  ...
  load(data) {
    ...
    this.titleDefault = "";
    if (Array.isArray(this.title.default) && this.title.default.length > 0) {
      this.titleDefault = this.title.default[0];
    }
    ...
    if (data.media) {
      this.thumbnail =
        data.media && data.media[0] && data.media[0].Thumbnail
    ...
    if (
      data.descriptiveData.backgroundImg &&
      data.descriptiveData.backgroundImg.Original &&
      data.descriptiveData.backgroundImg.Original.withUrl
    ) {
      this.background = data.descriptiveData.backgroundImg.Original.withUrl;
    } else if (
      data.descriptiveData.backgroundImg &&
      data.descriptiveData.backgroundImg.Thumbnail &&
      data.descriptiveData.backgroundImg.Thumbnail.withUrl
    ) {
  ...
  get isOwner() {
    return this.myAccess === "OWN";
  }
  get canEdit() {
    return this.myAccess === "OWN" || this.myAccess === "WRITE";
  }
  get hasDescription() {
